...
function computeCsvStructure(csvLines: string[]): CsvStructure {
  ...
  const availableHeaders = headerLine
    .split(",")
    .map((header) => header.trim())
    .filter((header) => header);
...
function parseLine(line: string, csvStructure: CsvStructure): Measure {
  ...
  const getColumnIndex = (key: string): number =>
    csvStructure.headersPerColumnIndex.get(key) as number;

  const date = new Date(
    values[getColumnIndex(KEY_DATE)] + " " + values[getColumnIndex(KEY_TIME)]
  );
  ...
  for (const [element, structure] of csvStructure.elements.entries()) {
    concentrations.push({
      ...
      value:
        values[structure.concentrationColumnIndex] === "<LOD"
          ? LOD
          : +values[structure.concentrationColumnIndex],
    });
  }
  return {
...
export function measuresFromCsv(csvContent: string): MeasureGroup[] {
  if (!csvContent) {
    throw new Error("No CSV content provided.");
  }
  ...
  groupPerMeasures.forEach((measures, name) =>
    measureGroup.push({ measures, name })
  );
